...
def create_samples(n_clusters, n_samples_per_cluster, n_features, embiggen_factor, seed):
    np.random.seed(seed)
    # Creates samples for each cluster
    slices = []
    centroids = []
    for i in range(n_clusters):
        # slice_start = [i*n_samples_per_cluster] + ([0] * (n_features-1))
        # slice_size = [n_samples_per_cluster] + ([-1] * (n_features-1))
...
def nearest_centroid(sample, centroids):
    ...
    d = tf.reduce_sum(tf.pow(tf.sub(tf.tile(sample, (k, 1)), centroids), 2))
    return tf.argmin(d)
